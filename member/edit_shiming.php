...
if($dopost=='save')
{
    ...
    if(is_uploaded_file($shiming))
    {
        ...
        //上传新工图片
        $shiming = MemberUploads('shiming', $oldshiming, $cfg_ml->M_ID, 'image', 'myface2', 600, 400);
    }
    else
    {
        $shiming = $oldshiming;
...
else if($dopost=='delold')
{
    ...
    $query = "UPDATE `#@__member` SET `shiming` = '' WHERE mid='{$cfg_ml->M_ID}' ";
    $dsql->ExecuteNoneQuery($query);
    // 清除缓存
    $cfg_ml->DelCache($cfg_ml->M_ID);
    ShowMsg('成功删除原来的实名信息！', $backurl);
    exit();
}
$shiming = $cfg_ml->fields['shiming'];
$xingming = $cfg_ml->fields['xingming'];
$shenfenzheng = $cfg_ml->fields['shenfenzheng'];

include(DEDEMEMBER."/templets/edit_shiming.htm");
exit();
?>