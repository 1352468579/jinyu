...
if($uid=='')
{
    ...
    else
    {
		...
		p.birthday,p.place,p.income,p.height,p.lovemsg
		FROM `#@__member` as m 
		left join `#@__member_person` as p on p.mid=m.mid 
		where m.matt!=10
		";

		function birthday2($birthday){   list($year,$month,$day) = explode("-",$birthday);   $year_diff = date("Y") - $year;   $month_diff = date("m") - $month;   $day_diff  = date("d") - $day;   if ($day_diff < 0 || $month_diff < 0)    $year_diff--;   return $year_diff; } 
...
else
{
		...
		$row2=$dsql->GetOne("SELECT  * FROM `#@__member` WHERE mid='{$_vars['mid']}'");
		$face = $row2['face'];
